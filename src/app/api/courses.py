...
async def get_courses(
    ...
    if filter.collaborator is not None:
        query = query.join(CourseCollaborators).filter((CourseCollaborators.accepted == True) & (CourseCollaborators.collaborator_id == filter.collaborator))
...
async def get_collaborators(course=Depends(check_course)):
    associations = session.query(CourseCollaborators).filter((CourseCollaborators.course_id == course.id) & (CourseCollaborators.accepted == True))
...
async def get_pending_collaborations(userId: UUID):
    associations = session.query(CourseCollaborators).filter((CourseCollaborators.collaborator_id == userId) & (CourseCollaborators.accepted == False))
...
async def add_collaborator(userId: UUID, course=Depends(check_course)):
    association = session.query(CourseCollaborators).filter((CourseCollaborators.course_id == course.id) & (CourseCollaborators.collaborator_id == userId)).first()
    if association is not None:
        return JSONResponse(status_code=status.HTTP_409_CONFLICT, content='User already invited.')
    session.add(CourseCollaborators(course_id=course.id, collaborator_id=userId, accepted=False))
    session.commit()
...
async def accept_collaborator(userId: UUID, course=Depends(check_course)):
    accepted = session.query(CourseCollaborators).filter((CourseCollaborators.course_id == course.id) & (CourseCollaborators.collaborator_id == userId) & (CourseCollaborators.accepted == True)).first()
    if accepted is not None:
        return JSONResponse(status_code=status.HTTP_409_CONFLICT, content='Collaborator already accepted.')

    invite = session.query(CourseCollaborators).filter((CourseCollaborators.course_id == course.id) & (CourseCollaborators.collaborator_id == userId) & (CourseCollaborators.accepted == False)).first()
    if invite is None:
...
async def remove_collaborator(userId: UUID, course=Depends(check_course)):
    association = session.query(CourseCollaborators).filter((CourseCollaborators.collaborator_id == userId) & (CourseCollaborators.course_id == course.id)).first()
    if association is None:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content='No collaborator found.')
    session.query(CourseCollaborators).filter((CourseCollaborators.collaborator_id == userId) & (CourseCollaborators.course_id == course.id)).delete()
    session.commit()
